...
(function(window, angular) {
    ...
    angular.module('fms').controller('AdminCtrl', ['$scope', '$state', '$q', 'DictionaryService', 'ParameterService', 'DialogManager',
        function($scope, $state, $q, DictionaryService, ParameterService, DialogManager) {
            ...
            $scope.changeParamsSortOrder = function(item, partFrom, partTo, indexFrom, indexTo) {
                if (indexTo === 0) {
                    item.orderIndex = partTo[0].orderIndex - 1;
                } else if (indexTo == partTo.length - 1) {
                    item.orderIndex = partTo[partTo.length - 1].orderIndex + 1;
                } else {
                    var prev = partTo[indexTo - 1].orderIndex;
                    var next = partTo[indexTo + 1].orderIndex;
                    item.orderIndex = (prev + next) / 2
                }                
                updateParam(item);
            }
            ...
            function updateParam(param) {
                param.isUpdating = true;
                return ParameterService.update(param).catch(showError).finally(function() {
                    delete param.isUpdating;
                });
            }
